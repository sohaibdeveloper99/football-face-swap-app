class AdvancedFaceSwapService {
  ...
  async loadModels() {
    ...
    try {
      ...
      // Load only the essential model for better performance
      ...
      console.log('Advanced face-api.js model loaded successfully');
      return true;
    } catch (error) {
  ...
  async detectFaces(imageElement) {
    ...
    try {
      ...
      console.log(`Detected ${detections.length} face(s) with high quality`);
      return detections;
    } catch (error) {
  ...
  async processJerseyImage(imageFile) {
    return new Promise((resolve, reject) => {
      ...
      img.onload = async () => {
        try {
          console.log('Processing jersey image with high quality...');
          ...
          // Remove faces from jersey with high quality
          const processedImageUrl = await this.removeFacesFromJersey(img, detections);
          
          // Extract face regions with proper sizing
          const faceRegions = detections.map(detection => {
            const box = detection.box;
            // Use exact face dimensions for precise replacement
            return {
              x: box.x,
              y: box.y,
              width: box.width,
  ...
  async processFaceImage(imageFile) {
    return new Promise((resolve, reject) => {
      ...
      img.onload = async () => {
        try {
          console.log('Processing face image with high quality...');
      ...
      const onImageLoad = async () => {
        ...
        if (loadedCount === 2) {
          try {
            ...
            // Overlay each face with high quality
  ...
  async overlaySingleFace(ctx, faceImg, jerseyRegion, faceRegion) {
    return new Promise((resolve) => {
      ...
      // Apply natural face mask
      this.applyNaturalFaceMask(tempCtx, faceWidth, faceHeight);
      
      // Draw the processed face to main canvas with maximum quality
...
export default new AdvancedFaceSwapService();