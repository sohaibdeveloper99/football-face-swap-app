class SimpleFaceSwapService {
  ...
  async loadModels() {
    ...
    try {
      ...
      console.log('Simple face-api.js model loaded successfully');
      return true;
    } catch (error) {
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = async () => {
        try {
          ...
          // Extract face regions with better positioning
          const faceRegions = detections.map(detection => {
            const box = detection.box;
            // Expand the face region to include more area for better positioning
            const expandedX = Math.max(0, box.x - box.width * 0.1);
            const expandedY = Math.max(0, box.y - box.height * 0.1);
            const expandedWidth = box.width * 1.2;
            const expandedHeight = box.height * 1.2;
      ...
      img.onload = async () => {
        try {
          ...
          const bestFace = detections.reduce((best, current) => {
            ...
            if (current.score === best.score && currentArea > bestArea) return current;
            return best;
          });
      ...
      const onImageLoad = async () => {
        ...
        if (loadedCount === 2) {
          try {
            ...
            // Overlay each face - use the entire face image
  ...
  async overlaySingleFace(ctx, faceImg, jerseyRegion) {
    return new Promise((resolve) => {
      // Calculate proper face positioning
      const destX = jerseyRegion.x;
      const destY = jerseyRegion.y;
      const destWidth = jerseyRegion.width;
      const destHeight = jerseyRegion.height;
      ...
      // Apply background removal to temp canvas
      this.removeBackgroundFromFace(tempCtx, faceImg.width, faceImg.height);
      
      // Apply natural face mask to temp canvas
      this.applyNaturalFaceMask(tempCtx, faceImg.width, faceImg.height);
...
export default new SimpleFaceSwapService();
