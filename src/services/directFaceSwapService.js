class DirectFaceSwapService {
  ...
  async loadModels() {
    ...
    try {
      ...
      console.log('Direct face-api.js model loaded successfully');
      return true;
    } catch (error) {
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = async () => {
        try {
          ...
          // Extract face regions with full hair only - no neck to avoid clothing
          const faceRegions = detections.map(detection => {
            const box = detection.box;
            // Expand the face region to include full hair only - no neck
            const hairPadding = 0.5; // 50% expansion for full hair coverage
            const neckPadding = 0; // No neck expansion to avoid clothing
            ...
            const expandedHeight = box.height * (1 + 2 * hairPadding + neckPadding); // No neck area
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = async () => {
        try {
          ...
          // Expand the best face region to include full hair only - no neck
          const bestBox = bestFace.box;
          const hairPadding = 0.5; // 50% expansion for full hair coverage
          const neckPadding = 0; // No neck expansion to avoid clothing
          const expandedBestX = Math.max(0, bestBox.x - bestBox.width * hairPadding);
          const expandedBestY = Math.max(0, bestBox.y - bestBox.height * hairPadding);
          const expandedBestWidth = bestBox.width * (1 + 2 * hairPadding);
          const expandedBestHeight = bestBox.height * (1 + 2 * hairPadding + neckPadding); // No neck area
      ...
      const onImageLoad = async () => {
        ...
        if (loadedCount === 2) {
          try {
            ...
            // Overlay each face - DIRECT REPLACEMENT
  ...
  async overlaySingleFace(ctx, faceImg, jerseyRegion, faceRegion) {
    return new Promise((resolve) => {
      ...
      // Draw the face area to temp canvas
      tempCtx.drawImage(
        faceImg,
      ...
      this.removeDarkBackground(tempCtx, faceWidth, faceHeight);
      
      // Apply sharp face mask - no soft edges
      this.applySoftFaceMask(tempCtx, faceWidth, faceHeight);
  ...
  removeDarkBackground(ctx, width, height) {
    ...
    applySoftFaceMask(ctx, width, height) {
      const imageData = ctx.getImageData(0, 0, width, height);
      const data = imageData.data;
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        // Create sharp elliptical mask for face and hair only
        const dx = (x - centerX) / radius;
        const dy = (y - centerY) / radius;
        const distance = Math.sqrt(dx * dx + dy * dy);
        
        // Sharp cut at 85% radius for clean edges
        if (distance > 0.85) {
          alpha = 0; // Sharp cut, no fade
        }
        
        // Sharp cut at chin level - no neck
        if (y > centerY + radius * 0.3) {
          alpha = 0; // Sharp cut at chin, no neck
        }
        
        // Sharp cut on sides for clean edges
        if (Math.abs(x - centerX) > radius * 0.8) {
          alpha = 0; // Sharp cut on sides
        }
...
export default new DirectFaceSwapService();
